...
export const reactive = function (target: any, obj: any) {
    ...
    const handler = {
        get(target: object, propertyKey: PropertyKey, receiver?: any) {
            ...
            // cc.log(`get--${String(propertyKey)} -- ${result}`);
...
function spriteFrameHandler(target: any, data: string) {
    return new Promise(async (res, rej) => {
        ...
        if (url.startsWith("http")) {
            ...
            cc.resources.load(url, cc.SpriteFrame, (e: Error, spriteFrame: cc.SpriteFrame) => {
                if (!!e) {
                    rej(`${url}加载失败！`);
                    return;
                }
...
function vforHandler(target: any, property: string, handle: vforType) {
    return new Promise(async (res, rej) => {
        try {
            ...
            let propertyIns: cc.Node = target[property];
